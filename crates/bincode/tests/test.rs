...
fn test_eos_serialization() {
    ...
    let blocks_request = Requests::GetBlocksRequest(GetBlocksRequestV0 {
        ...
        have_positions: vec![BlockPosition {
            block_num: 321956,
            block_id: Checksum256 { value: [9; 32] },
        }],
    ...
    assert_eq!(
        serialize(&blocks_request).unwrap(),
        vec![
            1, 10, 0, 0, 0, 13, 0, 0, 0, 255, 255, 255, 255, 1, 164, 233, 4, 0, 9, 9, 9, 9, 9, 9,
            9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 0, 1, 1,
            1
        ]
    );

    let optional_num: Option<u32> = Some(4294967295);
    assert_eq!(
        serialize(&optional_num).unwrap(),
